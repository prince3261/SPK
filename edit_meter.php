...
if (isset($_GET['meter_id']) || $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$meter) {
        echo "<script>alert('ไม่พบข้อมูลมิเตอร์'); window.location.href = 'search_meter.php';</script>";
        exit();
    }
}
...
if (isset($meter['room_id']) && isset($meter['meter_date'])) {
    ...
    $prev_meter_value = $prev_result->fetch_assoc()['meter_value'] ?? 0; // กำหนดค่า default เป็น 0
} else {
    $prev_meter_value = 0;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        if (empty($_POST['meter_value']) || empty($_POST['meter_rate']) || empty($_POST['meter_date'])) {
        ...
        $meter_month_thai = getThaiMonth($meter_month); // แปลงเดือนเป็นภาษาไทย
        ...
        echo "<script>alert('แก้ไขข้อมูลมิเตอร์สำเร็จ'); window.location.href='search_meter.php?meter_id=$meter_id';</script>";
    } catch (Exception $e) {
        $conn->rollback();
